#!/usr/bin/env bash
# Dò giới hạn dung lượng video của MyLife (bucket mylife-video) bằng cách
# mô phỏng đúng flow resumable upload: POST start -> PUT bytes.
# Rule chỉ chặn theo size nên dùng data giả (/dev/zero) là đủ.
#
# Dùng: ID_TOKEN='<token>' ./probe-mylife-video-limit.sh
set -uo pipefail

ID_TOKEN="${ID_TOKEN:?Set ID_TOKEN env var}"
UID_ML="ubnDy27WfjdtwPXglvWD5WRFdnu1"
TMP="$(mktemp -d)"
trap 'rm -rf "$TMP"' EXIT
...
auth=(-H "Authorization: Firebase ${ID_TOKEN}" -H "X-Ios-Bundle-Identifier: com.mylife.MyLife")

# try_size <bytes> : đặt LAST_CODE = HTTP code của PUT; append CREATED_NAMES nếu 200
try_size() {
  local size="$1"
  local name; name="$(rand_name).mp4"
  local path="users/${UID_ML}/moments/videos/${name}"
  local enc="users%2F${UID_ML}%2Fmoments%2Fvideos%2F${name}"
  local base="https://firebasestorage.googleapis.com/v0/b/mylife-video/o/${enc}?uploadType=resumable&name=${enc}"
  ...
  hdr="$(curl -s -D - -o /dev/null -X POST "$base" "${auth[@]}" \
    ...
    --data "{\"name\":\"${path}\",\"contentType\":\"video/mp4\",\"bucket\":\"\",\"metadata\":{\"creator\":\"${UID_ML}\",\"visibility\":\"private\"}}")"
...
for enc in "${CREATED_NAMES[@]:-}"; do
  [[ -z "$enc" ]] && continue
  curl -s -o /dev/null -X DELETE "https://firebasestorage.googleapis.com/v0/b/mylife-video/o/${enc}" "${auth[@]}"
  echo "  deleted: $enc"
done

# Liệt kê (CHỈ ĐỌC) các object trong thư mục videos để em tự rà object sót từ lần chạy trước.
echo "== Object hiện có trong users/${UID_ML}/moments/videos/ (chỉ liệt kê, không xoá) =="
python3 - "$ID_TOKEN" "$UID_ML" <<'PY'
import sys, json, urllib.request
tok, uid = sys.argv[1], sys.argv[2]
base="https://firebasestorage.googleapis.com/v0/b/mylife-video/o"
r=urllib.request.Request(f"{base}?prefix=users/{uid}/moments/videos/",
    headers={"Authorization":f"Firebase {tok}","X-Ios-Bundle-Identifier":"com.mylife.MyLife"})
